...
const BOS_ITEMS = [
  { sku: 'HOP_TIN_KIT_4P',      qty: 6,  reason: '22 panels ÷ 4 per kit = 6 kits' },
  ...
  { sku: 'GEN_RCK_SEAL_EPD_B',  qty: 22, reason: 'One EPDM roof seal per panel mount' },
...
const DISCOUNT_PCT = 5;   // fixed customer discount across all tiers
const all = Object.values(TIERS).map(costTier);
console.log('\n');
console.log('═'.repeat(210));
console.log(`  SUMMARY — all 3 tiers (incl GST) · ${DISCOUNT_PCT}% customer discount applied`);
console.log('═'.repeat(210));
console.log(`  ${pad('TIER', 56)} ${pad('Major HW', 12, 'right')} ${pad('BoS', 12, 'right')} ${pad('Labour', 12, 'right')} ${pad('SUBTOTAL', 13, 'right')} ${pad(`-${DISCOUNT_PCT}% DISC`, 12, 'right')} ${pad('FINAL PRICE', 14, 'right')} ${pad('HW Margin $', 13, 'right')} ${pad('HW Margin %', 13, 'right')} ${pad('HW Cost', 12, 'right')}`);
console.log('  ' + '─'.repeat(208));
for (const r of all) {
  const discount = +(r.totalSellInc * DISCOUNT_PCT / 100).toFixed(2);
  const finalPrice = +(r.totalSellInc - discount).toFixed(2);
  // Margin after discount: the 5% comes out of margin first (cost is fixed),
  // so adjusted hardware margin = totalHwMargin - (discount × hardware-share-of-quote)
  // For accuracy: discount applies pre-GST, so discount-ex-GST eats margin.
  const discountExGst = +(discount / 1.15).toFixed(2);
  const adjustedMargin = +(r.totalHwMargin - discountExGst).toFixed(2);
  const adjustedMarginPct = +((adjustedMargin / r.hwCost) * 100).toFixed(1);
  console.log(`  ${pad(r.tier.label.slice(0, 56), 56)} ${pad(fmt$0(r.majorSellInc), 12, 'right')} ${pad(fmt$0(r.bosSellInc), 12, 'right')} ${pad(fmt$0(r.labourSellInc), 12, 'right')} ${pad(fmt$0(r.totalSellInc), 13, 'right')} ${pad('-' + fmt$0(discount), 12, 'right')} ${pad(fmt$0(finalPrice), 14, 'right')} ${pad(fmt$0(adjustedMargin), 13, 'right')} ${pad(adjustedMarginPct + '%', 13, 'right')} ${pad(fmt$0(r.hwCost), 12, 'right')}`);
}
console.log('═'.repeat(210));
console.log(`  Note: HW Margin columns reflect margin AFTER absorbing the ${DISCOUNT_PCT}% discount (discount comes off your margin, not added to cost).`);
